...
void rehash() {
    ...
    for (int i = 0; i < 26; i++) {
        curr = oldTable[i];
        while (curr != NULL) {
            pushBack(hashFuction(curr->data), curr->data);
...
void pushBackLinearProbing(int hashKey, char data[]) {
    ...
    while (hashTable[hashKey] != NULL) {
        ...
        if (hashKey == originalHashKey) {
            ...
            rehash();
            pushBackLinearProbing(hashFuction(data), data);
            return;
        }
    }
    struct node* newNode = (struct node*)malloc(sizeof(struct node));
...
int main()
{
    ...
    memset(hashTable, 0, sizeof(hashTable));
    int run = 1;
    int (*hashFunction)(int) = divHash; // default hash function
    ...
    while (run != 0)
    {
        ...
        printf("6. View Hash Table\n");
        printf("7. Set Collision Handling Method\n");
        printf("8. Exit\n");
        ...
        do
        {
            ...
            pushBackWithCollisionHandling(hashFunction(key), data, collisionMethod);
